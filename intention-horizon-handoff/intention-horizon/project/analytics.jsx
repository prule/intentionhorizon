/* ───────────────────────────────────────────────
   analytics.jsx — insights: heatmap, filter, totals, CSV
   ─────────────────────────────────────────────── */
...
function Heatmap({ filter, version }) {
  const t = IH.today();
  const WEEKS = 13;
...
function Cell({ cell, heatColor, delay }) {
  if (!cell) return <div style={{ aspectRatio: '1', borderRadius: 7 }} />;
  const { count, metRatio, targetedTotal } = cell;
  const has = count > 0;
...
function Legend({ heatColor, filter }) {
  return (
...
function TotalsBlock({ filter, version }) {
  const [grouping, setGrouping] = React.useState('day');
  const ranges = { day: 14, month: 186, year: 730 };
  const data = IH.aggregate(filter, grouping, ranges[grouping]).filter(d => grouping !== 'day' ? true : true);
  const shown = grouping === 'day' ? data.slice(-10) : data;
  ...
  return (
    <div style={{ padding: '20px 20px 0' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        <div style={{ width: 188 }}>
          <Segmented small value={grouping} onChange={setGrouping}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
        {shown.map((d, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 64, fontSize: 12, color: 'var(--ink-3)', textAlign: 'right', flexShrink: 0 }} className={grouping === 'day' ? '' : ''}>{d.label}</div>
      ...
      <div style={{ marginTop: 12, fontSize: 12, color: 'var(--ink-3)' }}>
        <span className="num" style={{ fontWeight: 600, color: 'var(--ink-2)' }}>{totalSum}</span> completions shown
      </div>
    </div>
  );
}

function StreakStats({ filter, version }) {
  const { current, best, rate } = IH.streaks(filter);
  const tiles = [
    { label: 'Current streak', value: current, unit: current === 1 ? 'day' : 'days', flame: true },
    { label: 'Best run', value: best, unit: best === 1 ? 'day' : 'days' },
    { label: '30-day rate', value: rate, unit: '%' },
  ];
  return (
      ...
      {tiles.map((t, i) => (
        <div key={i} style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', boxShadow: 'var(--shadow-card)', padding: '13px 12px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 3 }}>
            ...
            <span className="num" style={{ fontSize: 26, fontWeight: 700, lineHeight: 1, letterSpacing: '-0.02em' }}>{t.value}</span>
...
function AnalyticsScreen({ version }) {
  const s = IH.load();
  ...
  return (
    <div className="ih-scroll fade-up" style={{ paddingBottom: 28 }}>
      ...
      <StreakStats filter={filter} version={version} />
      <Heatmap filter={filter} version={version} />
      <TotalsBlock filter={filter} version={version} />
    </div>
  );
}
