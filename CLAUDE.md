...
**Keeping the generated instruction files in sync:** the `/opsx:*` commands and `openspec-*` skills under `.claude/commands/opsx/`, `.claude/skills/`, `.agent/workflows/`, and `.agent/skills/` are generated by the OpenSpec CLI, not hand-written — they drift out of date whenever the installed CLI (pinned as `OPENSPEC_VERSION` in `.devcontainer/Dockerfile`) is upgraded. After bumping that pin, run `openspec update` from the repo root and commit the regenerated files alongside the version bump; skipping this leaves the committed instructions describing an older CLI's behavior.
