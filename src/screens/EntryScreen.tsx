    ...
    <Sheet open={open} onClose={onClose} title="Jump to month">
      {periods.map((p) => (
        <button
          key={`${p.year}-${p.month}`}
          className="ih-btn"
          data-testid="month-picker-option"
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 20px 14px' }}>
      <button
        ...
        disabled={!canPrev}
        onClick={() => canPrev && setDate(IH.addDays(date, -1))}
      ...
      <button
        ...
        disabled={!canNext}
        onClick={() => canNext && setDate(IH.addDays(date, 1))}
...
function IntentionRow({
  ...
  return (
    <div
      data-testid="intention-row"
      data-intention-name={intention.name}
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
        <CircleToggle
          on={on}
          color={color}
          testid="intention-toggle"
          onToggle={() => {
            IH.toggleCompletion(intention.id, key);
            onChange();
          }}
        />
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          <Stat
            count={periodCount}
            target={intention.targetEnabled ? intention.targetCompletions : null}
            label={`${intention.targetPeriodDays}d`}
            testid="stat-target"
          />
        </div>
      </div>
...
export function EntryScreen({
  ...
  const groups = [
    ...
    ...(intentsByCat['_none'] ? [{ id: '_none', name: 'Uncategorized' }] : []),
  ...
  return (
    ...
    <div className="ih-scroll fade-up" data-testid="screen-entry" style={{ paddingBottom: 24 }}>
      ...
      >
        <ScreenHeader
          title="Journal"
          subtitle={`${doneToday} of ${total} intentions complete`}
          right={
            openGuide ? (
              <button
                className="ih-btn"
        ...
        <div key={g.id} style={{ marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 24px 7px' }}>
            ...
            <span className="num" style={{ fontSize: 11, color: 'var(--ink-4)' }}>
              {
                IH.doneOnDay(IH.dateKey(date)).filter((it) => (it.categoryId || '_none') === g.id)
                  .length
              }
              /{(intentsByCat[g.id] || []).length}
            </span>
          </div>
          <div
            style={{
          ...
          >
            {(intentsByCat[g.id] || []).map((it, i, arr) => (
              <React.Fragment key={it.id}>
                ...
                {i < arr.length - 1 && (
                  <div style={{ height: 1, background: 'var(--line-soft)', marginLeft: 63 }} />
                )}
              </React.Fragment>
            ))}
          </div>
      ...
      {s.intentions.length === 0 && (
        ...
        >
          No intentions yet. Head to <strong style={{ color: 'var(--ink-2)' }}>Manage</strong> to
