  ...
  return (
    <button
      ...
      style={{
        ...
        padding: 3,
        flexShrink: 0,
        background: on ? 'var(--c-moss)' : 'var(--line)',
...
function Stepper({
  ...
  return (
    <div style={{ flex: 1 }}>
      <div style={{ fontSize: 11.5, color: 'var(--ink-3)', marginBottom: 6, fontWeight: 600 }}>
        {label}
      </div>
      <div
      ...
      >
        {btn(
          ...
          value > min,
          testid ? `${testid}-dec` : undefined,
        )}
        <span
          className="num"
          data-testid={testid ? `${testid}-value` : undefined}
          style={{ fontSize: 18, fontWeight: 600, minWidth: 24, textAlign: 'center' }}
        >
          {value}
        </span>
        {btn(
...
function IntentionEditor({
  ...
  open: boolean;
  onClose: () => void;
  editing: IH.Intention | null;
  categories: IH.Category[];
  onSaved: () => void;
}) {
  const blank: IH.IntentionInput = {
    name: '',
  ...
  const save = () => {
    if (!valid) return;
    if (editing) IH.updateIntention(editing.id, form);
    else IH.addIntention(form);
    onSaved();
    onClose();
  };
  ...
  return (
    <Sheet
      open={open}
      onClose={onClose}
      title={editing ? 'Edit intention' : 'New intention'}
      footer={
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <PrimaryButton
              onClick={save}
              disabled={!valid}
              color={IH.colorVar(form.color)}
              testid="save-intention"
            >
              {editing ? 'Save changes' : 'Create intention'}
            </PrimaryButton>
          </div>
        </div>
      }
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 12.5, color: 'var(--ink-3)', marginTop: 1 }}>
              How many times within a period
            </div>
          </div>
          <Switch
            on={form.targetEnabled}
            onToggle={() => set({ targetEnabled: !form.targetEnabled })}
            testid="targets-switch"
          />
        </label>
        {form.targetEnabled && (
          <div style={{ display: 'flex', gap: 12, padding: '0 16px 16px', alignItems: 'flex-end' }}>
            <Stepper
              ...
              min={1}
              max={99}
              onChange={(v) => set({ targetCompletions: v })}
              testid="target-completions"
            />
            <div style={{ flex: 1 }}>
              <div
                style={{ fontSize: 11.5, color: 'var(--ink-3)', marginBottom: 6, fontWeight: 600 }}
              >
                Period (days)
              </div>
              <input
                className="ih-input num"
                data-testid="target-period"
                type="number"
                inputMode="numeric"
                min={1}
                value={periodText}
...
function CategoryEditor({
  ...
  const save = () => {
    if (!valid) return;
    if (editing) IH.updateCategory(editing.id, name.trim());
    else IH.addCategory(name.trim());
    onSaved();
    onClose();
  };
  return (
    <Sheet
      ...
      footer={
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <PrimaryButton onClick={save} disabled={!valid} testid="save-category">
    ...
    >
      ...
      {editing && (
        <div style={{ fontSize: 12.5, color: 'var(--ink-3)', lineHeight: 1.5 }}>
          Deleting a category keeps its intentions and moves them to Uncategorized.
        </div>
      )}
    </Sheet>
  );
  ...
  return (
    <button
      ...
      style={{
        ...
        fontSize: 15,
        fontWeight: 600,
        color: danger ? 'var(--c-rose)' : 'var(--ink-2)',
      }}
    >
      {icon}
      {label}
...
function DataSourceSwitcher({ bump }: { bump: () => void }) {
  ...
  const pick = (src: IH.DataSource) => {
    if (src !== active) IH.switchDataSource(src).then(bump);
  };
  return (
    <div
      style={{
    ...
    >
      ...
      <div style={{ fontSize: 12.5, color: 'var(--ink-3)', marginTop: 1 }}>
        Mock is sample data; Real is your own. Kept separate.
      </div>
      <div
        style={{
          display: 'flex',
  ...
  return (
    ...
    >
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 12.5, color: 'var(--ink-3)', marginTop: 1 }}>
          Anonymous analytics cookies to improve the app. Your intentions never leave this device.
        </div>
      </div>
...
export function SettingsScreen({
  ...
  const s = IH.load();
  const [intentEditor, setIntentEditor] = React.useState<{
    open: boolean;
    editing: IH.Intention | null;
  }>({ open: false, editing: null });
  const [catEditor, setCatEditor] = React.useState<{ open: boolean; editing: IH.Category | null }>({
    open: false,
    editing: null,
  });
  ...
  const onImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      !confirm(
        'Import this CSV into the Real data source? New categories, intentions, and completed days will be merged into your existing data.',
      )
    )
      return;
    ...
    alert(
      `Import complete. Added ${res.categoriesAdded} categor${res.categoriesAdded === 1 ? 'y' : 'ies'}, ${res.intentionsAdded} intention(s), ${res.daysAdded} completed day(s).${skipped}`,
    );
  };
    ...
    return (
      <div style={{ marginBottom: 18 }}>
        <div
          ...
          data-category-name={g.name}
          style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '0 22px 7px' }}
        >
          {!noDrag && (
            <span
              {...handle!}
          ...
          {g.id !== '_none' && (
            <button
              ...
              data-category-name={g.name}
              onClick={() => setCatEditor({ open: true, editing: g })}
              style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--ink-3)' }}
            >
              Edit
            </button>
          )}
        ...
        >
          {rows.length === 0 && (
            <div style={{ padding: '14px 16px', fontSize: 13.5, color: 'var(--ink-4)' }}>
              No intentions
            </div>
          )}
          <DragList
              ...
              return (
                <div
                  style={{
                    ...
                    alignItems: 'center',
                    background: 'var(--surface)',
                    borderBottom: last || dragging ? 'none' : '1px solid var(--line-soft)',
                  }}
                >
                  <span
                    {...h}
                  ...
                  <button
                    className="ih-btn"
                    data-testid="settings-intention"
                    data-intention-name={it.name}
                    onClick={() => openIntent(it)}
                    style={{
                      flex: 1,
                      display: 'flex',
                      alignItems: 'center',
                  ...
                  >
                    <span
                      style={{
                        ...
                        height: 14,
                        borderRadius: 5,
                        background: IH.colorVar(it.color),
                        flexShrink: 0,
                      }}
                    />
                    <span
                    ...
                    {it.targetEnabled && (
                      ...
                      >
                        {it.targetCompletions}/{it.targetPeriodDays}d
                      </span>
                    )}
  ...
  return (
    <div className="ih-scroll fade-up" data-testid="screen-settings" style={{ paddingBottom: 28 }}>
      <ScreenHeader
        title="Manage"
        subtitle={`${s.intentions.length} intentions · ${s.categories.length} categories`}
        right={
          <button
            className="ih-btn"
            data-testid="add-intention"
            onClick={() => setIntentEditor({ open: true, editing: null })}
            style={{
              width: 40,
              height: 40,
              borderRadius: 12,
      ...
      <div style={{ padding: '4px 14px 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <RowButton
          label="Add category"
          testid="add-category"
          onClick={() => setCatEditor({ open: true, editing: null })}
          icon={<Icon.plus style={{ width: 18, height: 18 }} />}
        />
        <RowButton
          label="Export data as CSV"
          testid="export-csv"
          onClick={() => IH.downloadCSV()}
          icon={<Icon.download />}
        />
        <RowButton
        ...
        {IH.mockEnabled() && IH.getDataSource() === 'mock' ? (
          <RowButton
            ...
            onClick={() => {
              if (confirm('Reset mock data to the seeded sample set?')) {
                IH.resetSeed();
                bump();
              }
            }}
          ...
          <RowButton
            ...
            testid="clear-data"
            onClick={() => {
              if (confirm('Clear all data in the Real source? This cannot be undone.')) {
                IH.resetSeed();
                bump();
              }
            }}
      ...
      >
        {formatVersion(__APP_VERSION__, __GIT_SHA__)}
      </div>

      <IntentionEditor
        open={intentEditor.open}
        editing={intentEditor.editing}
        categories={s.categories}
        onClose={() => setIntentEditor({ open: false, editing: null })}
        onSaved={bump}
      />
      <CategoryEditor
        open={catEditor.open}
        editing={catEditor.editing}
        onClose={() => setCatEditor({ open: false, editing: null })}
        onSaved={bump}
      />
    </div>
  );
