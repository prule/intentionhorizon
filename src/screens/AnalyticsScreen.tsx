...
function Cell({
  ...
  return (
    <div
      title={`${IH.MONTHS[cell.day.getMonth()]} ${cell.day.getDate()} · ${count} done`}
      style={{
        aspectRatio: '1',
        borderRadius: 7,
        position: 'relative',
        ...
        placeItems: 'center',
        background: has
          ? `color-mix(in oklab, ${heatColor} ${Math.round(op * 100)}%, var(--surface))`
          : 'var(--bg-2)',
    ...
    >
      {has && (
        <span
          className="num"
          style={{ fontSize: 12, fontWeight: 600, color: dark ? '#fff' : 'var(--ink-2)' }}
        >
          {count}
        </span>
      )}
        ...
        {[0.18, 0.45, 0.72, 1].map((r, i) => (
          <div
            ...
            style={{
              ...
              height: 15,
              borderRadius: 4.5,
              background: `color-mix(in oklab, ${heatColor} ${Math.round((0.22 + 0.78 * r) * 100)}%, var(--surface))`,
...
function Heatmap({ filter }: { filter: string | null }) {
  const t = IH.today();
  const WEEKS = 13;
  const heatColor = filter
    ? IH.colorVar(IH.load().intentions.find((i) => i.id === filter)?.color || 'teal')
    : 'var(--c-teal)';
    ...
    <div style={{ padding: '4px 20px 0' }}>
      ...
      >
        ...
        {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
          <div
            key={i}
            style={{ textAlign: 'center', fontSize: 10, fontWeight: 700, color: 'var(--ink-4)' }}
          >
            {d}
          </div>
        ))}
        ...
        {weeks
          ...
          .map((row, wi) => {
            ...
            return (
              <div
                key={wi}
                style={{ display: 'grid', gridTemplateColumns: '22px repeat(7, 1fr)', gap: 5 }}
              >
                <div
                  style={{
                    fontSize: 9,
                ...
                {row.map((c, di) => (
                  <Cell key={di} cell={c} heatColor={heatColor} delay={(wi * 7 + di) * 4} />
                ))}
              </div>
            );
          })}
      ...
      >
        ...
        <div style={{ width: 188 }}>
          <Segmented
            small
            value={grouping}
            onChange={(v) => setGrouping(v as IH.Grouping)}
            options={[
              { value: 'day', label: 'Day' },
      ...
      {tiles.map((t, i) => (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 3 }}>
            {t.flame && current > 0 && (
              <Icon.flame
                style={{ width: 16, height: 16, color: 'var(--c-clay)', alignSelf: 'center' }}
              />
            )}
            <span
              className="num"
...
export function AnalyticsScreen({ version }: { version: number }) {
  ...
  React.useEffect(() => {
    if (filter && !s.intentions.find((i) => i.id === filter)) setFilter(null);
  }, [version]);
