...
export function Stat({
  ...
  const status =
    target == null ? null : count < target ? 'under' : count === target ? 'on' : 'above';
    ...
    >
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 1 }}>
        <span
          ...
          style={{ fontSize: 17, fontWeight: 600, color: valColor, lineHeight: 1 }}
        >
          {count}
        </span>
        {target != null && (
  ...
  return createPortal(
    <div className="ih-sheet-scrim">
      ...
      <div className="ih-sheet">
        ...
        <div className="ih-scroll" style={{ padding: '0 20px 16px', flex: 1 }}>
          {children}
        </div>
        {footer && (
          <div
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: '14px',
        borderRadius: 14,
        background: disabled ? 'var(--line)' : color,
      ...
      {items.map((it, i) => {
        ...
        return (
          <div
            key={getKey(it)}
            ref={(el) => {
              if (el) itemRefs.current[getKey(it)] = el;
            }}
            style={{
              position: 'relative',
              zIndex: dragging ? 20 : 1,
              transform: dragging
                ? `translateY(${drag!.delta}px) scale(1.015)`
