...
The Insights view presents aggregated completion data — the Totals block (buckets grouped by Day, Month, or Year) and the heatmap — in most-recent-first order so the data the user cares about most is at the top. Day grouping is capped to the ten most recent buckets to keep the block focused and readable.
