...
Detecting that a newer deployed version is available, prompting the user in-app, applying the update (activate waiting worker + reload) on demand, and suppressing the prompt after dismissal until the next deploy.
