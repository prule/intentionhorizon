...
Defines the project's unit-testing capability: a headless Vitest runner that exercises store logic without a real browser, the coverage it must provide over the store's CSV and core computation, and the per-test isolation guarantees that keep tests independent and reliable.
