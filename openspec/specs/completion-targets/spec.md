...
Defines how an intention's target is expressed (N completions over M days), how completion counts are measured against it, and how target status (under/on/above) is derived for the Entry and Analytics screens. A single, user-defined target — `targetCompletions` within `targetPeriodDays` — replaces the former fixed 7-day/30-day pair, covering common goals like "three times per week" or "once a year" with one concept.
