...
Defines the presentation behavior of the shared modal dialog (`Sheet` component): a bottom sheet on mobile and a centered modal on desktop. Covers the backdrop scrim (blur, coverage) and the panel's rounded-corner clipping so dialogs such as Add Category and Add Intention render correctly.
