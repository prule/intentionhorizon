...
Defines how the app imports data from a CSV file in the export long-format (`date,category,intention,completed`), reconstructing categories, intentions, and completions into the active `real` data source. Import merges by name, unions completions, defaults fields the export format omits, and validates atomically so an invalid file never mutates the existing store.
