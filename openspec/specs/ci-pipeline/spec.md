...
The project SHALL provide a GitHub Actions pipeline that, on every push to the default branch and on every pull request, builds the app and runs the end-to-end suite. The pipeline SHALL fail (exit non-zero / report a failing check) if the build or any e2e test fails.

#### Scenario: Pull request runs the checks

- **WHEN** a pull request is opened or updated
- **THEN** the pipeline runs the build and the e2e suite and reports a status check on the pull request
...
- **THEN** the pipeline runs the build and the e2e suite before any deployment step
