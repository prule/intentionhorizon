...
Computing an app version and short commit hash from git history at build time, injecting them into the bundle, and displaying them on the Manage screen with a safe fallback when git metadata is absent.
