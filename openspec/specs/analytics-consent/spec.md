...
The app is local-first and its only third-party data flow is Google Analytics 4 (gtag.js). Analytics SHALL be off by default and only enabled with the user's explicit, revocable consent, implemented via Google Consent Mode v2. The choice persists across reloads and can be changed at any time from the Manage screen.
